...
def update_line_chart(pick_type):
    ...
    fig = px.line(data_array_diff, x = data_dict_diff['Wavelength'][0], y=data_array_diff, title='Molecule Spectral Radience Differance from Standard Titan Atmosphere', log_y=True)
    for i, newname in enumerate(Ratio[np_mask]):
        fig.data[i].name = newname
    return fig # .write_html("SR.html")
